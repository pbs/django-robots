    ...
    operations = [
        migrations.CreateModel(
            ...
            fields=[
                ...
                ('robot', models.CharField(default=b'*', help_text="This should be a user agent string like 'Googlebot'. Enter an asterisk (*) for all user agents. For a full list look at the <a target=_blank href='http://www.robotstxt.org/db.html'> database of Web Robots</a>.", max_length=255, verbose_name='robot')),
                ('crawl_delay', models.DecimalField(default=5.0, help_text="Between 0.1 and 99.0. This field is supported by some search engines and defines the delay between successive crawler accesses in seconds. If the crawler rate is a problem for your server, you can set the delay up to 5 or 10 or a comfortable value for your server, but it's suggested to start with small values (0.5-1), and increase as needed to an acceptable value for your server. Larger delay values add more delay between successive crawl accesses and decrease the maximum crawl rate to your web server.", verbose_name='crawl delay', max_digits=3, decimal_places=1)),
            ],
            options={
                'verbose_name': 'rule',
        ...
        migrations.AddField(
            model_name='rule',
            name='disallowed',
            field=models.ManyToManyField(help_text="List URL paths that are not allowed to be accessed by bots (You don't need to disallow every URL, as it'll block anything matching a string starting with a listed path.).", related_name='disallowed', verbose_name='disallowed', to='robots.Url', blank=True),
            preserve_default=True,
        ),
        migrations.AddField(
            model_name='rule',
